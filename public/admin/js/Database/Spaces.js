Ext.define('Admin.Database.Spaces', {
    ...
    truncateSpace(space, index, key,searchCount) { // эвент обработки нажатия кнопки truncate
        //TODO здесь проверка выбирали ли мы данные по индексу
console.log(searchCount);


        // если мы что-то выбрали в поиск
        if (index != null) {
            ...
            var test = Ext.apply({
              ...
              Ext.MessageBox.confirm({
                ...
                message: 'u  chose find ' + space + '?<br/>Delete selected  '+ searchCount +' rows ?',
                buttons: Ext.MessageBox.YESNO,
                callback: (answer) => {
                    if (answer == 'yes') {
                        //dispatch('space.truncate',this.spaceParams1(space,index,key) )
                        dispatch('space.truncate', test)
                    }
                }
            })
        } else {
    ...
    tbar: [{
        ...
        disabled: true,
        handler() {
            var params = {space: this.up('database-spaces').truncateSpace(this.up('grid').getSelectionModel().getSelection()[0].get('name'))}
        }
    }, {
        text: 'Drop',
        name: 'drop-button',
